  ...
  return (
    <Tooltip title={Title}>
      <CollectionItemContainer>
        <CollectionImage
          className="image"
          imageUrl={Poster === "N/A" ? noImage : Poster}
        />
        <CollectionFooterContainer>
          <NameContainer>
        ...
        {getNominationsCount > 4 ? (
          <Message>Can't choose more than 5!</Message>
        ) : null}
        <AddButton inverted onClick={handleOnclick}>
          {getNominationsCount > 4 ? "Go to Nominations" : "Nominate"}
        </AddButton>
      </CollectionItemContainer>
    </Tooltip>
  );
